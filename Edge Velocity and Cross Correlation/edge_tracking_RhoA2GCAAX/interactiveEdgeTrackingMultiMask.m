...
for frameNum=1:length(filenamesFRET)
    ...
    cellCoorsAll = [];
    maskCombined = zeros(size(imRFP_raw));
    ...
    for c = 1:numCells;
        ...
        bgCFP = regionprops(maskring, imCFP,'PixelValues');
        bgCFPAv = nanmean(bgCFP(:));
        bgFRET = regionprops(maskring, imFRET,'PixelValues');
        bgFRETAv = nanmean(bgFRET(:));
        %bg subtraction???
