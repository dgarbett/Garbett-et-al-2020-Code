...
clear; close all; clc;
datadir='/Volumes/BisackUp/Meyer_Lab/CDH5-TS-Thrombin/CDH5-TS-1min_1/data';
position='Pos5';
%% Parameters for cell edge parametrization
nFretWindows=200;                                   % Number of windows to use for FRET vs edge correlation measurements
...
subplot(1,2,1);imagesc(protvalsWindow);
...
vw.set('Quality',100);
open(vw);
RangeProtVals=[round(prctile(protvalsWindow(:),3)) round(prctile(protvalsWindow(:),97))];
