...
%% calculate size vectors and preallocate memory
n=cellfun(@(x) size(x,1),pnt);
%n=cellfun(@length,pnt);
%make sure you have enough trajectories in case no cell can be tracked
%(from cumsum)
ncmsm=cumsum([0 n]);
ncmsm=ncmsm(:);
...
%         % the seeds
%         M=searchsingle(double(arg.seed'),double(pnt{1}(:,arg.dim)'),arg.maxdisp);
%         match{1}(:,1)=double(M(:,1));
%         for i=2:frameNum
%             %get our the x,y coordinates 
...
if ~isempty(connectTraj)
    ...
    cnt=0;
    traj2connect=zeros(ncmsm(end),1);
    %if nonzero element
    while any(possibleStart)
        %increment count
        cnt=cnt+1;
