...
%% Preparation
clear; clc; close all;
root = 'K:\data\141121_NG';
alignFolder=[root filesep 'alignment40x'];

datadir=([root filesep,'data_150520']);
mkdir(datadir);
...
% 
% imraw=imread(['K:\data\141121_NG\CC-RhoA_15s_1\Pos0\img_000000000_EPI-FRET (dual view)_000.tif']);
...
% [mask cellCoors]=getCellMask(imSum);
% 
% % Detrmine ratio
% imFRETbg(~mask)=nan;
...
for frameNum=1:(length(filenames));
    ...
    imSum=double(imCFP_raw+imFRET_raw);
    [mask cellCoorsTemp]=getCellMask(imSum,4000);
...
%     mask=imfilter(mask_raw,fspecial('disk',2),'symmetric');
    ...
    imRatio{frameNum}=imRatioTemp;
%   imRatioScaled(~mask)=0;
     if frameNum==1
        colorRange = [round(prctile(imRatioTemp(:),1),1),round(prctile(imRatioTemp(:),95),1)];
     end
...
%     imRatioStack(:,:,frameNum)=imRatio;
    ...
    imwrite(imFRETOutline,[datadir filesep dataSubdir{subNum},'_pos_',num2str(position),'_FRET.tif'],'WriteMode','append','Compression','none');
    %imwrite(tempDAPI,[root filesep 'data' filesep position,'_DAPI.tif'],'WriteMode','append','Compression','none');
    imwrite(tempRATIO,[datadir filesep dataSubdir{subNum},'_pos_',num2str(position),'_RATIO_',num2str(colorRange(1)),'_',num2str(colorRange(2)),'.tif'],'WriteMode','append','Compression','none');
   
end
save([datadir filesep dataSubdir{subNum},'_pos_',num2str(position),'_RatioData.mat'],'maskFinal', 'cellCoors', 'imRatio');
disp('done!');
