...
clear; close all; clc;
datadir='K:\data\150620_NG\CDH5-Ruby_CC-Rac1_1min_2\data_150712'
position='Pos0';
...
edgeDepthDist=5; %was 7                                   % Number of pixels deep for the windows for computing FRET values.
...
protvalsWindowShift=[protvalsWindow(150:360,:); protvalsWindow(1:149,:)];
fretvalsShift=[fretvals(150:360,:); fretvals(1:149,:)];

protvalsWindowF=ndnanfilter(protvalsWindowShift,fspecial('disk',2),'replicate');
fretvalsF=ndnanfilter(fretvalsShift,fspecial('disk',2),'replicate');
...
%%
subplot(1,2,1); plot(edgeCoors{1}(:,1),edgeCoors{1}(:,2));
subplot(1,2,2); plot(edgeCoorsSmoothed{1}(:,1),edgeCoorsSmoothed{1}(:,2));
